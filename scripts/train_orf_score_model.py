...
def _load_genome(path: Path, args: argparse.Namespace):
    ...
    if args.table is not None:
        ...
        if genome_table not in SUPPORTED_TABLES:
            print(f"Warning: unsupported table {genome_table} in {path}, skipping", file=sys.stderr)
            return None
    ...
    try:
        ...
        print(f"Warning: find_orfs failed for {path}: {exc}", file=sys.stderr)
        return None

    rows = []
    labels = []
    for orf in orfs:
        rows.append(orf.extract_features())
...
def _print_cv_summary(cv_result: dict) -> None:
    ...
    print("-" * 70)
    print(f"{'Fold':>5} {'N_train':>8} {'N_test':>8} {'Pos':>6} {'Prec':>7} {'Rec':>7} {'AUC':>7}")
    print("-" * 70)
    metrics = []
...
def main() -> int:
    ...
    parser.add_argument(
        "--min-orf-len", type=int, default=90,
    ...
    print(f"Wrote {args.output}: {total_pos} positive, {total_rows - total_pos} negative examples.")
    return 0
